...
namespace SpiroWeb.Handlers
{
    ...
    /// </summary>
    public class GetBusinessProductImage : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            try
            {
                ...
                if (context.Request.QueryString["Id"] != null)
                {
                    ...
                    //else
                    //{
                    int _idValue = int.Parse(_Id);
                    var _UserProductsSimple = db.BusinessProducts.Where(c => c.Id == _idValue).FirstOrDefault();
                    if (_UserProductsSimple != null)
                    {
                        if (_UserProductsSimple.Image != null)
                        {
                            //See if is to resize
                            if (context.Request.QueryString["width"] != null && context.Request.QueryString["height"] != null)
                            {
                                ...
                                Image _resizedImage = SpiroWeb.Helpers.Imager.PutOnWhiteCanvas(SpiroWeb.Helpers.Imager.byteArrayToImage(_UserProductsSimple.Image), _width, _height);
                            ...
                            context.Response.OutputStream.Write(_UserProductsSimple.Image, 0, _UserProductsSimple.Image.Length);
                            context.Response.ContentType = "image/JPEG";
                            return;
                        }
                    }
                    //return default.png
                    string _imageUrl = HttpContext.Current.Server.MapPath("~/Media/Images/default.png");
                    if (System.IO.File.Exists(_imageUrl))
                    {
                        byte[] _imageFromDisk = GetImageInBytes(_imageUrl);
                        context.Response.OutputStream.Write(_imageFromDisk, 0, _imageFromDisk.Length);
